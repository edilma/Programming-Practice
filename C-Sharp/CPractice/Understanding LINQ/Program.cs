...
namespace Understanding_LINQ
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /*
            var someCars = from car in myCars
                           where car.Make == "BMW" || car.Year==2009
                           select car;

            //var someCars = myCars.Where(p => p.Make == "BMW" || p.Year==2009);
            ...
            Console.WriteLine($"The first {orderedCars.Make} car is {orderedCars.Model} {orderedCars.Year}");*/

            //Console.WriteLine(myCars.TrueForAll(p=> p.Year>2000));

            myCars.ForEach(p => p.StickerPrice -= 3000);

            myCars.ForEach(p => Console.WriteLine($"{p.Make} and {p.Year} cost {p.StickerPrice:C}" ));

            /*
            foreach (var car in orderedCars)
            {
                Console.WriteLine($"This is my car {car.Make}: {car.Model} , {car.Year} ");
            }
